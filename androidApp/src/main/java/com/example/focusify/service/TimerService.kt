...
package com.example.focusify.service
...
import com.example.focusify.di.ActivityCallbacks
...
class TimerService : Service(), KoinComponent {
    ...
    private fun updateProgressSegments() {
        val settingsState = _settingsState.value
        val selectedTaskName = _timerState.value.selectedTaskName
        val sessionLength = if (selectedTaskName != null) {
            runBlocking {
                taskRepository.getTaskByName(selectedTaskName)?.totalPomodoros ?: settingsState.sessionLength
            }
        } else settingsState.sessionLength
            ...
            .also {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.BAKLAVA && !settingsState.singleProgressBar) {
                    ...
                    for (i in 0..<sessionLength * 2) {
                        ...
                        else if (i != (sessionLength * 2 - 1)) it.addProgressSegment(
                            NotificationCompat.ProgressStyle.Segment(
    ...
    private suspend fun resetTimer() {
        val settingsState = _settingsState.value
        val selectedTaskName = _timerState.value.selectedTaskName
        val task = if (selectedTaskName != null) taskRepository.getTaskByName(selectedTaskName) else null
        val sessionLength = task?.totalPomodoros ?: settingsState.sessionLength
        ...
        _timerState.update { currentState ->
            currentState.copy(
                ...
                timeStr = millisecondsToStr(time),
                totalTime = time,
                nextTimerMode = if (sessionLength > 1) TimerMode.SHORT_BREAK else TimerMode.LONG_BREAK,
                nextTimeStr = millisecondsToStr(if (sessionLength > 1) settingsState.shortBreakTime else settingsState.longBreakTime),
                currentFocusCount = 1,
                totalFocusCount = sessionLength
            )
        }
    ...
    private suspend fun skipTimer(fromButton: Boolean = false) {
        ...
        val task = if (selectedTaskName != null) taskRepository.getTaskByName(selectedTaskName) else null
        val sessionLength = task?.totalPomodoros ?: settingsState.sessionLength
        val currentTimerMode = _timerState.value.timerMode

        if (currentTimerMode == TimerMode.FOCUS) {
            if (!fromButton) {
                task?.let {
                    val updatedTask = it.copy(
                        completedPomodoros = it.completedPomodoros + 1,
                        isCompleted = it.completedPomodoros + 1 >= it.totalPomodoros
                    )
                    taskRepository.updateTask(updatedTask)

                    if (updatedTask.isCompleted) {
                        _timerState.update { currentState ->
                            currentState.copy(selectedTaskName = null)
                        }
                        resetTimer()
                        return
                    }
                ...
                // If focus session skipped manually, mark task as skipped but not completed
                task?.let {
                    val updatedTask = it.copy(isSkipped = true)
                    taskRepository.updateTask(updatedTask)
                }
            }
        }

        cycles = (cycles + 1) % (sessionLength * 2)
        ...
        if (cycles % 2 == 0) {
            ...
            _timerState.update { currentState ->
                currentState.copy(
                    ...
                    timeStr = millisecondsToStr(time),
                    totalTime = time,
                    nextTimerMode = if (cycles == (sessionLength - 1) * 2) TimerMode.LONG_BREAK else TimerMode.SHORT_BREAK,
                    nextTimeStr = if (cycles == (sessionLength - 1) * 2) millisecondsToStr(
            ...
            val long = cycles == (sessionLength * 2) - 1
    ...
    enum class Actions {
        TOGGLE, SKIP, RESET, UNDO_RESET, STOP_ALARM, UPDATE_ALARM_TONE, SELECT_TASK
    }
}
